...
def mix_roidbs_for_training(dataset_names, proposal_files, use_charanns):
    """Load and concatenate roidbs for one or more datasets, along with optional
    object proposals. The roidb entries are then prepared for use in training,
    which involves caching certain types of metadata for each roidb entry.
    """
...
def flip_polygons(polygons, width):
    if polygons is not None:
        oldx1 = polygons[:, 0].copy()
        oldx2 = polygons[:, 2].copy()
        oldx3 = polygons[:, 4].copy()
        oldx4 = polygons[:, 6].copy()
        oldy1 = polygons[:, 1].copy()
        oldy2 = polygons[:, 3].copy()
        oldy3 = polygons[:, 5].copy()
        oldy4 = polygons[:, 7].copy()
        polygons[:, 0] = width - oldx2 - 1
        polygons[:, 1] = oldy2
        polygons[:, 2] = width - oldx1 - 1
        polygons[:, 3] = oldy1
        polygons[:, 4] = width - oldx4 - 1
        polygons[:, 5] = oldy4
        polygons[:, 6] = width - oldx3 - 1
        polygons[:, 7] = oldy3
    return  polygons

def flip_word(word):
    #['hello', 'h', 'e', 'l', 'l', 'o'] -> ['olleh', 'o', 'l', 'l', 'e', 'h']
